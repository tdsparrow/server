  ...
  return new Promise(function(resolve, reject) {
    ...
    if (arrDictionaries[id]) {
      ...
      if (arrExistDictionaries[id]) {
        let pathTmp = path.join(
          pathDictionaries,
          allLanguages[id],
          allLanguages[id] + "."
        );
        dict = arrDictionaries[id] = new nodehun(
          pathTmp + "aff",
          pathTmp + "dic"
        );
      }
    ...
    if (dict) {
      if ("spell" === type) {
        ...
        dict.spellSuggestions(word, function(
          err,
          correct,
          suggestions,
          origWord
...
exports.install = function(server, callbackFunction) {
  ...
  sockjs_echo.on("connection", function(conn) {
    ...
    conn.on("data", function(message) {
      logger.info("sockjs message %s", message);
      try {
    ...
    sendData(conn, {
      type: "init",
      languages: Object.keys(arrExistDictionaries).concat(["2052"])
    });
  });
